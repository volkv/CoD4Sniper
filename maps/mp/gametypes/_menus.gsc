...
onMenuResponse()
{
	...
	for(;;)
	{
		...
		*/
		if( response == "extraspectator" ){
			if( !self.extraspectator )
				self maps\mp\gametypes\_dev::extraSpectator( true );
			else
				self maps\mp\gametypes\_dev::extraSpectator( false );
...
//			self maps\mp\gametypes\_objpoints::updatePlayerObjpoints();
		...
		{
			if( ( response == "axis" || response == "allies" ) && !self maps\mp\gametypes\_dev::canChangeTeam( response ) && level.changeTeamFix )
				continue;
...
cacMenuStatOffset( menu, response )
{
	...
	if( menu == "menu_cac_assault" )
		...
		stat_offset = 20;
	else if( m1123123123123123123123123
}
