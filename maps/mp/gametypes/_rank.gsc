...
initScoreHud( fullUpdate )
{
	...
	if( !level.newScoreUpdate )
	{
		...
		self.hud_rankscroreupdate = maps\mp\gametypes\_dev::newScoreUpdate();
}
...
giveRankXP( type, value, text )
{
	...
	if( level.newScoreUpdate ){
		multiplier = 1;
		if( getDvar( "tmax_xpMultiplier" ) == "" )
			multiplier = 10;
		else
			multiplier = getDvarFloat( "tmax_xpMultiplier" );
	...
	if ( isDefined( self.enableText ) && self.enableText && !level.hardcoreMode )
	{
		if ( type == "teamkill" )
			...
			self thread updateRankScoreHUD( value, text );
	}
...
updateRankScoreHUD( amount, text )
{
	self endon( "disconnect" );
	...
	{			
		...
		else
		{
			...
			if( level.newScoreUpdate ){
				self.hud_rankscroreupdate.color = (0.9843,0.9254,0.3647);
				self.hud_rankscroreupdate.underline.color = (0.9843,0.9254,0.3647);
			}
			else
		...
		if( level.newScoreUpdate ){
			self.hud_rankscroreupdate.underline setText( text );
			self.hud_rankscroreupdate.alpha = 1;
			self.hud_rankscroreupdate.underline.alpha = 1;
		}
		else {
		...
		self.hud_rankscroreupdate.underline fadeOverTime( 0.75 );
		self.hud_rankscroreupdate.underline.alpha = 0;
